...
class PluginRepository {
	...
	def tracePlugins(head: List[Plugin], permit: Feature => Boolean) = {
		def trace(past: List[Plugin], present: List[Plugin]) = {
	...
	def sortPlugins(plugins: List[Plugin]) = {
		def iterate(free: List[Plugin], edges: List[(Plugin, Plugin)], sofar: List[Plugin]) = {
			val n = free.head
			...
			nowfree match {
				case Nil => 
					if (!edges.empty) {
						...
						throw new Exception("Cyclical graph: unsatisfied edges between "+errors.mkString(", ")
					}
					Nil
...
class PluginContext (features: List[Feature], plugins: List[Plugin]) {
	...
	val registeredHooks: HashMap[Hook, List[(Int, Any)]]
	...
	def get[S](hook: Hook[S]) = 
		registry(hook).getOrElse(List()).map(_._2.asInstanceOf[S])
}